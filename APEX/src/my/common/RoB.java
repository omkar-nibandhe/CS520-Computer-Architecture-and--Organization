...
public final class RoB extends AbstractCollection<InstructionModel> {
  ...
  public RoB(int c, int mc) {
    ...
    bitmask = capacity - 1;
    q = new InstructionModel[capacity];
  }
  ...
  public boolean add(InstructionModel obj) {
    if (size == capacity) {
      // no room
      return false;
    }
  ...
  public InstructionModel remove() {
    InstructionModel obj;
  ...
  public Iterator<InstructionModel> iterator() {
    return new Iterator<InstructionModel>() {
      ...
      public InstructionModel next() {
        checkForModification();
        if (s == 0) throw new NoSuchElementException();
    
        s--;
        InstructionModel r = q[i];
        i = (i + 1) & bitmask;
